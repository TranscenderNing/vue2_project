<template>
  <div class="clearfix selector">
    ...
    <div class="type-wrap" v-for="(attr,index) in attrsList" :key="attr.attrId">
      ...
      <div class="fl value">
        <ul class="type-list">
          <li v-for="(attrdValue,index) in attr.attrValueList" :key="index" @click="handleAttrInfo(attr,attrdValue)">
            <a>{{ attrdValue }}</a>
          </li>
        </ul>
      </div>
  ...
  export default {
    ...
    methods:{
      tradeMarkHandler(trademark){
        console.log('点击了品牌',trademark)
        this.$emit('getTradeMarkInfo',trademark)
      },
      handleAttrInfo(attr,attrdValue){
        console.log("点击了attr info")
        this.$emit('getAttrInfo',attr,attrdValue)
