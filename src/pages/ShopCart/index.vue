      ...
      <div class="cart-body">
        <ul class="cart-list" v-for="(cart,index) in cartInfoList" :key="cart.id">
          <li class="cart-list-con1">
            <input type="checkbox" name="chk_list" :checked="cart.isChecked" @change="modifyChecked(cart,$event)">
          </li>
          ...
          <li class="cart-list-con7">
            <a href="#none" class="sindelet" @click="deletCart(cart.skuId)">删除</a>
            <br>
            <a href="#none">移到收藏</a>
          </li>
        </ul>
...
import throttle from 'lodash/throttle'
  ...
  export default {
    ...
    computed:{
      ...
      totalPrice(){
        let sum = 0
        this.cartInfoList.forEach(elem => {
          sum += elem.skuNum * elem.skuPrice
        });
        return sum
      },
      isCheckAll(){
    ...
    methods:{
      ...
      // async changeNum(type,disNum,cart){
      ...
      //   }
      //   try{
      //     let res = await this.$store.dispatch('AddOrUpdateCart',{skuId:cart.skuId, skuNum: disNum})
      //     this.getData()
      //   }catch{

      //   }
      // },
      async deletCart(skuId){
        try{
          let res = await this.$store.dispatch('deletCart',skuId)
      ...
      async modifyChecked(cart,event){
        try{
          let checked = event.target.checked ? '1':'0'
          let res = await this.$store.dispatch('modifyChecked',{skuId: cart.skuId, isChecked: checked})
          this.getData()
        }catch{
          alert('修改checked失败')
        }
