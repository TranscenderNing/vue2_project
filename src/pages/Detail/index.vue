    ...
    methods:{
      ...
      changeNum(event){
        ...
        if(isNaN(userInput) || userInput < 1){
          ...
          this.skuNum = parseInt(userInput)
        }
      },
      async addToCart(){
        try{
          // 向服务器发送请求， 服务器保存 购物车 相关信息
          let res = await this.$store.dispatch('AddOrUpdateCart',{skuId:this.$route.params.skuid, skuNum:this.skuNum})
          console.log('fuwuqi',res)
          // 路由跳转 需要将产品信息带到跳转的组件中
          // 简单数据 可以 query传递
          // 复杂数据可以通过 会话存储 可以存对象
