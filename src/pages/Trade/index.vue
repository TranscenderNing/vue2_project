  ...
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div class="address clearFix" v-for="(address,index) in addressList" :key="address.id">
        ...
        <p @click="changeDefalut(address,addressList)">
      ...
      <div class="detail">
        <h5>商品清单</h5>
        <ul class="list clearFix" v-for="(order,index) in orderList.detailArrayList" :key="order.skuId">
          <li>
    ...
    <div class="trade">
      ...
      <div class="receiveInfo">
        寄送至:
        <span>{{userRealAddress.fullAddress}}</span>
        收货人：<span>{{ userRealAddress.consignee}}</span>
        <span>{{  userRealAddress.phoneNum }}</span>
      </div>
    </div>
...
import { getSubmitOrder } from '@/api'
  export default {
    name: 'Trade',
    data(){
      return {
    ...
    computed:{
      ...
      userRealAddress(){
        let res = this.addressList.find(item => item.isDefault == 1)
        return res || {}
      }
    },
    methods:{
      changeDefalut(address,addressList){
        addressList.forEach(element => {element.isDefault = 0});
        address.isDefault = 1
      },
      async submitOrder(){
        let { tradeNo }  = this.orderList
        let data = {
          consignee: this.userRealAddress.consignee,
          consigneeTel: this.userRealAddress.phoneNum,
          deliveryAddress: this.userRealAddress.fullAddress,
          paymentWay: "ONLINE",
          orderComment: this.msg,
          orderDetailList: this.orderList.detailArrayList
       }
        let res = await this.$API.getSubmitOrder(tradeNo,data)
        if (res.code === 200){
